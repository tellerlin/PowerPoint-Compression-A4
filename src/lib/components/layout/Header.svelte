    ...
    <nav class="flex items-center justify-between h-16">
      ...
      <div class="hidden md:flex items-center space-x-8">
        {#each navItems as item}
          <a
            href={item.href}
            class="text-gray-600 hover:text-gray-900 hover:bg-gray-50 px-3 py-2 rounded-md text-sm font-medium transition-colors duration-150
                   {$page.url.pathname === item.href ? 'text-primary-600' : ''}"
          >
            {item.label}
          </a>
        {/each}
